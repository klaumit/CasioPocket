...
void drawListBox(TListBox *l)
{
	...
	LibLine(l->x + 1, l->y,            l->w - 2, 1, 1);
	LibLine(l->x + 1, l->y + l->h - 1, l->w - 2, 1, 1);
	LibLine(l->x,            l->y + 1, 1, l->h - 2, 1);
	LibLine(l->x + l->w - 1, l->y + 1, 1, l->h - 2, 1);
...
int LBClicked(TCHSTS *tsts, TListBox *l)
{
	...
	if ((tsts->act & ACT_SCR_BAR) != 0 && tsts->x >= (l->sc.x-2)) {
        ...
        /* DEBUG */
        sprintf(buf,"%d %d %d",tsts->y, l->y, ih);
        sprintf(buf,"%d %d %d",line, l->sc.pos, l->sc.vol);
