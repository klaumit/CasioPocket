...
extern unsigned long far newint22;
extern unsigned char far hookend;
extern unsigned long far orgint22vec;
...
unsigned short TestInstalled()
{
  ...
  p = MK_FP(0x0015,0x0000); /* aka 0000:0150 */
  ...
  if (n)
  {
    ...
    q = &newint22;
    if (far_memcmp(p,q,FP_OFF(&orgint22vec)) == 0)
    {
          /* return installed mode */
      return ((unsigned short far *)MK_FP(0x0015,FP_OFF(&mode)+1))[0];
    }
...
int RemoveHook()
{
  ...
  if (q[0] != 0x00150000)
  {
    return 0;
      ...
      /* restore vector */
  p = MK_FP(0x0015,FP_OFF(&orgint22vec)+1);
  q[0] = p[0];
...
void InstallHook(unsigned short nMode)
{
  ...
  p = MK_FP(0x0000,0x0088);
  q = MK_FP(0x0015,FP_OFF(&orgint22vec)+1);
...
void main()
{
  ...
  else if (nMode != -1)
  {
    ...
    InstallHook(nMode);
    LibPutMsgDlg("Autostart installed.");
  }
